...
+ *Question 1*
+ Assume that the two short sides of a right triangle have length 3 and 4. What is the length of the long side?
+ Write a BSL expression that produces the the value of ? for this triangle where the other two sides have lengths 3 and 4.
#+BEGIN_SRC racket
(sqrt (+ (sqr 3) (sqr 4)))
#+END_SRC
=> 5
...
+ *To evaluate a function call* src_racket{(bulb (string-append "re" "d"))}
  1. First, reduce operands to values, also known as *arguments.*
     ...
     + The expression now looks like this: src_racket{(bulb "red")}
